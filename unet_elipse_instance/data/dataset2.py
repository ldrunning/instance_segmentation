...
class CellSeg(dataset.Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(self.image_list[item]).convert("RGB")  # .resize((960, 960))
        # cyt = Image.open(self.cyt_list[item]).convert("L")
        nuc = Image.open(self.nuc_list[item]).convert("L")  # .resize((960, 960))
        ...
        if self.transform:
            img, nuc = self.transform(image=img, segmentation_maps=nuc)
        nuc = nuc.get_arr()
...
class SingleImage(dataset.Dataset):
    ...
    def __getitem__(self, item):
        img = Image.open(self.image_list[item])  # .resize((960, 960))
        img = ((np.array(img, dtype=np.float32) - 128) / 128.0).transpose((2, 0, 1))
        return img
...
if __name__ == '__main__':
    data = CellSeg("/home/khtao/data/DataSetv2/cell_segmention")
    one, two = data[0]
    print(len(data))
